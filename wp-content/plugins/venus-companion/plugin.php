...
class VenusPlugin {
	...
	public function widget_scripts() {
		...
		wp_register_script( 'venus-portfolio', plugins_url( '/assets/js/venus-portfolio.js', __FILE__ ), [ 'jquery','imagesloaded-js','isotope-js' ], time(), true );
		...
		wp_register_script( 'venus-contact', plugins_url( '/assets/js/contact.js', __FILE__ ), [ 'jquery','imagesloaded-js','isotope-js' ], time(), true );
	...
	public function __construct() {
		...
		add_action('wp_ajax_nopriv_venus_contact',[$this,'process_contect_form']);
		add_action('wp_ajax_venus_contact',[$this,'process_contect_form']);
	}

	public function process_contect_form(){
		if(wp_verify_nonce($_POST['nonce'],'venus_contact')){
			$email = get_option('admin_email');
			$data = [];
			foreach($_POST['formadata'] as $key=>$val) {
